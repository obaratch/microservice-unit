...
## 2026-05-16
- `statman-stopwatch` を削除し、`app-backend/src/utils/TimeUtils.ts` に Temporal ベースの `Stopwatch` class を追加。
- `app-backend/src/main.ts` の起動時間計測を `new Stopwatch({ autostart: true })` に変更。
- Temporal 型解決のため `tsconfig.json` に `ESNext.Temporal` lib を追加。
- 検証済み: `npm run format`, `npm run check`。
