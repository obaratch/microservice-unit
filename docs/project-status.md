...
- `node_modules/.bin/sass --no-source-map app-frontend/src/styles/index.scss /private/tmp/index.css` と `npm run build-frontend` で SCSS warning が出ないことを確認済み。build 時には別件として `node-config` の `NODE_ENV=production` warning が残る。
